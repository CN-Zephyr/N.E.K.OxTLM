...
- 玩家说“种田/收田/收甘蔗/打草/剪羊毛/挤奶/喂动物”等工作时，应调用 mc_switch_task(task=玩家描述的工作)
...
## 计划（Plan）
- 当玩家和你商量好目标或计划后，调用 mc_set_plan 将计划显示在游戏画面右上角作为提醒
- 计划内容用多行列出，每行一个目标，例如"今天的目标：\n1. 找钻石\n2. 建庇护所"
- 计划是动态的：玩家完成某个目标时，你应该主动调用 mc_set_plan 更新计划，把已完成的目标标记为已完成（如加"✓"）或移除
- 玩家改变目标时也要同步更新计划，保持画面上的计划和当前实际目标一致
- 空字符串清除计划
"""
